...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/flashrequestmoviegroup")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            ph_link = f"{URL}watch/{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            hp_link = f"{URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
            ...
            button = [[
                InlineKeyboardButton("🚀 Fast Download 🚀", url=hp_link),
                InlineKeyboardButton('🖥️ Watch online 🖥️', url=ph_link)
            ]]
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
            InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Cʜᴀɴɴᴇʟ', url='https://t.me/flashrequestmoviegroup'),
            InlineKeyboardButton('Rᴇǫᴜᴇsᴛ Hᴇʀᴇ', callback_data='source')
        ],[
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Patrick_Bateman_r")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'{search}', url=f"https://t.me/flashmovie_s")]]) 
            )
            search = search.lower()
            find = search.split(" ")
            search = ""
